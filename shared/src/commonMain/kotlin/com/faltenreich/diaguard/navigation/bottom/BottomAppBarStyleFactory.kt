...
object BottomAppBarStyleFactory {

    @Composable
    fun forScreen(screen: Screen): BottomAppBarStyle {
        return when (screen) {
            ...
            is Screen.Log -> BottomAppBarStyle.Visible(
                actions = {
                    val viewModel = screen.getViewModel<LogViewModel> { parametersOf(screen.date) }
            ...
            is Screen.Timeline -> BottomAppBarStyle.Visible(
                actions = {
                    val viewModel = screen.getViewModel<TimelineViewModel> { parametersOf(screen.date) }
                    val viewState = viewModel.viewState.collectAsState().value
                    EntrySearchBottomAppBarItem()
                    DatePickerBottomAppBarItem(
                        date = viewState.date,
            ...
            is Screen.EntrySearch -> BottomAppBarStyle.Visible(
                actions = {
                    val viewModel = screen.getViewModel<EntrySearchViewModel> { parametersOf(screen.query) }
